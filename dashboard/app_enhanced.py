...
def extract_actions_from_model_metadata(model_path):
    """
    Extract action labels from ONNX model metadata.
    Falls back to reading from data directory if metadata is not available.
    """
    try:
        # Try to extract actions from model metadata first
        import onnx
        ...
        if model.metadata_props:
            ...
            # Look for MSL actions metadata
            if 'msl_actions' in metadata:
                actions_json = metadata['msl_actions']
                actions = json.loads(actions_json)
                st.success(f"✅ Loaded {len(actions)} actions from model metadata")
                return np.array(actions)
            else:
                st.warning("⚠️ No MSL actions metadata found in model")
        else:
            st.warning("⚠️ No metadata found in model")
    
    except Exception as e:
        st.warning(f"⚠️ Failed to read model metadata: {e}")
    
    # Fallback to reading from data directory (original behavior)
    fallback_data_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data')
    try:
        actions = np.array([d for d in os.listdir(fallback_data_path) 
                          if os.path.isdir(os.path.join(fallback_data_path, d))])
        st.info(f"ℹ️ Loaded {len(actions)} actions from data directory (fallback)")
        return actions
    except FileNotFoundError:
        st.error(f"❌ Data directory '{fallback_data_path}' not found.")
        return np.array(['Gesture_1', 'Gesture_2'])
...
MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'model', 'bigger_with_metadata.onnx')
...
if model:
    # Extract actions from model metadata (with fallback to data directory)
    actions = extract_actions_from_model_metadata(MODEL_PATH)
    ...
    with col1:
        ...
        # Simplified WebRTC configuration without custom TURN servers (like example.py)
        webrtc_streamer(
            key="sign-language", 
